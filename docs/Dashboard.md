# 📊 Campaign Dashboard

Welcome to the Command Center for the **Dragon Delves** campaign.

---

## 🎯 Active Quest: The Infusion

> **Current Goal**: Investigate the vision of the ship at sea struck by green lightning and the monks in the sea cave.

| Progress   | Status              | Milestone                   |
| :--------- | :------------------ | :-------------------------- |
| **3 / 10** | Collecting Essences | Brass, Gold, Silver secured |

---

## 👥 The Fellowship

| Hero        | Role               | Status   | D&D Beyond                                                   |
| :---------- | :----------------- | :------- | :----------------------------------------------------------- |
| **Saffi**   | Scout / Ranger     | Healthy  | [View Sheet](https://www.dndbeyond.com/characters/149615029) |
| **Alamira** | Arcanist / Wizard  | Rested   | [View Sheet](https://www.dndbeyond.com/characters/150291260) |
| **Tra'Ahn** | Vanguard / Fighter | Alert    | [View Sheet](https://www.dndbeyond.com/characters/150299176) |
| **Caelen**  | Healer / Cleric    | Battered | [View Sheet](https://www.dndbeyond.com/characters/150298338) |

---

## 🏰 Base Status: The Nest

- **Location**: Off the coast of **Cloakwood**.
- **Security**: Guarded by **Uno** (Modron).
- **Facilities**:
  - 🌿 Garden (managed by **Cuatro**)
  - 🧪 Alchemy Lab (managed by **Dos**)
  - 📜 Library (under restoration)
- **Prisoners**: **Arlen Wymond** (Restrained).

---

## 🎒 Key Shared Loot

- 🐉 **Eggsmirelda**: Glowing with potential.
- 🗡️ **Leafwarden**: Ready for action.
- 👁️ **Staff of Invisibility**: Newly acquired from Shadowfell.
- 💎 **Giant Opal**: Value 1,000g.
